...
def train(hyp, opt, device, tb_writer=None):
    ...
    save_dir, epochs, batch_size, total_batch_size, weights, rank, kpt_label = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.batch_size, opt.weights, opt.global_rank, opt.kpt_label
    ...
    # DDP mode
    if cuda and rank != -1:
        opt.local_rank = -1  # 设置opt.local_rank为-1，取消分布式数据并行模式
        model = DDP(model, device_ids=[0])  # 设置device_ids为[0]，指定使用第一个GPU
...
if __name__ == '__main__':
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        opt.cfg, opt.weights, opt.resume, opt.batch_size, opt.global_rank, opt.local_rank = \
            '', ckpt, True, opt.batch_size, *apriori  # reinstate
